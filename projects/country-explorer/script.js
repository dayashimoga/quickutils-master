...
function showDetail(name){
  ...
  $('#detailContent').innerHTML=`
    ...
    <div class="detail-header"><div class="detail-flag">${c.flag}</div><div class="detail-title"><h1>${esc(c.name)}</h1><div class="subtitle">${esc(c.official)}</div><button class="btn btn-sm ${isFav?'btn-primary':'btn-secondary'}" id="favBtn">${isFav?'⭐ Favorited':'☆ Add Favorite'}</button></div></div>
...
function renderWorldMap(){
  ...
  // Region centroids (approx lat/lon mapped to canvas)
  const regionCenters={Africa:{lat:5,lon:20},Americas:{lat:15,lon:-80},Asia:{lat:35,lon:90},Europe:{lat:50,lon:15},Oceania:{lat:-25,lon:140}};
  ...
  // Plot each country as a dot with jitter
  let selectedDot=null;
  const dots=[];
  countries.forEach((c,i)=>{
    const rc=regionCenters[c.region];if(!rc)return;
    const jx=(Math.sin(i*137.5)*30)+(Math.cos(i*97)*20);
    const jy=(Math.cos(i*137.5)*25)+(Math.sin(i*53)*15);
    const pos=toXY(rc.lat+jy*0.3,rc.lon+jx*0.5);
    const r=Math.max(3,Math.min(12,Math.sqrt(c.population/5000000)));
    dots.push({x:pos.x,y:pos.y,r,country:c});
    ctx.fillStyle=regionColors[c.region]||'#6366f1';
    ctx.globalAlpha=0.7;
    ctx.beginPath();ctx.arc(pos.x,pos.y,r,0,Math.PI*2);ctx.fill();
    ctx.globalAlpha=1;
  });
  // Region labels
  ctx.font=`bold ${W/40}px Inter,sans-serif`;ctx.textAlign='center';
  Object.entries(regionCenters).forEach(([name,{lat,lon}])=>{
    const p=toXY(lat,lon);
    ctx.fillStyle=regionColors[name];ctx.globalAlpha=0.8;
    ctx.fillText(name,p.x,p.y-W/30);ctx.globalAlpha=1;
  });
  // Click handler
  cvs.onclick=function(e){
    ...
    if(closest&&minD<30){
      $('#mapInfo').innerHTML=`${closest.country.flag} <strong>${closest.country.name}</strong> — ${closest.country.capital} | Pop: ${fmt(closest.country.population)} | ${closest.country.currencies}`;
      // Highlight
      renderWorldMap();
      ctx.strokeStyle='#fff';ctx.lineWidth=3;ctx.beginPath();ctx.arc(closest.x,closest.y,closest.r+5,0,Math.PI*2);ctx.stroke();
      ctx.fillStyle='#fff';ctx.font=`bold ${W/50}px Inter`;ctx.textAlign='center';ctx.fillText(closest.country.name,closest.x,closest.y-closest.r-10);
    }
  };
}
