...
class TestFormatPost:
    ...
    def test_contains_title(self, sample_items):
        ...
        assert "NYC Taxi" in post

    def test_contains_description(self, sample_items):
        post = format_post(sample_items[0])
        assert "Taxi trip records" in post

    def test_contains_url(self, sample_items):
        post = format_post(sample_items[0])
        assert "/item/nyc-taxi-and-limousine-commission-trip-record-data.html" in post

    def test_contains_hashtags(self, sample_items):
        post = format_post(sample_items[0])
        assert "#OpenData" in post
        assert "#MachineLearning" in post
    ...
    def test_contains_license_info(self, sample_items):
        post = format_post(sample_items[0])
        assert "License: Public Domain" in post
