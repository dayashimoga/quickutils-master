...
5. **Configure the Project**
   ...
   - **No code changes needed** — the template conditionally loads GA only when a real ID is present.
...
1. **Sign Up for AdSense**
   ...
   - Select your country → Accept terms
...
4. **How It Works**
   - `build_directory.py` contains `BOOK_RECOMMENDATIONS` → a dict mapping API categories to curated books
   ...
   - Books appear in the item page sidebar under "📚 Recommended Books"
...
2. **Create an Application (API Token)**
   ...
   - Click **Submit**
   - Open the created application → copy **Your access token**
...
4. **Test Manually**
   - Go to **Actions** → **Social Media Bot** → **Run workflow**
   - Check your Mastodon profile for the post

5. **Customize Posts**
   - Edit `scripts/post_social.py` → `format_post()` function
...
3. **SSL Certificate**
   ...
   - Go to Settings → Domain management → HTTPS → Verify DNS → Provision certificate
...
4. For custom token, grant these permissions:
   ...
   - **Zone** → **DNS** → **Edit** (if managing DNS via Cloudflare)
...
Push to `main` → the `.github/workflows/cloudflare-pages.yml` workflow automatically:
...
2. Go to **Domain Management** → click `quickutils.top` → **DNS Records**
...
| **Limitation** | No server-side build — need GitHub Actions to build first |
...
| **Setup** | [vercel.com](https://vercel.com) → Import project → Set build command |
...
| **Setup** | [render.com](https://render.com) → New → Static Site → Connect repo |
...
| `MASTODON_ACCESS_TOKEN` | For bot | — | GitHub repo secrets |
